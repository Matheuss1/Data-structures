...
int main()
{
   ...
   for (i = 0; i < m; i++) {
      ...
      // this is the graph representation  for the tower
      ...
      for (j = 0; j < n; j++) {
         ...
         - vertexs with 'P' markers represents the exchange between lifts
...
p_node *bfs(p_graph tower, int towerSize)
{
   ...
   // lists of a single Node
   p_node topFloorA = pushList(newList(), towerSize - 1, 'P', 'P');
   p_node topFloorB = pushList(newList(), towerSize - 2, 'P', 'P');
   ...
   // visits all neighbors of the node V in the adj list until finds NULL pointer
      while (iterator != NULL) {
         int pos = getNodeValue(iterator);

         if (!visited[pos]) {
            visited[pos] = 1;
            // the parent is the floor of node V and the lift used until iterator floor value is
            // the markers of iterator itself
            parent[pos] = pushList(newList(), getNodeValue(v), getMarker(iterator), getMarker2(iterator));
            push(queue, iterator);
            ...
            if (getNodeValue(iterator) == 0) {
               ...
               // returns the parent array with the nodes that correponds to the path from the 
